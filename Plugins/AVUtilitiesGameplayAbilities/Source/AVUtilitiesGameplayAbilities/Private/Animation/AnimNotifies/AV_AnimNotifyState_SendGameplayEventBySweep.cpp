...
#include "Animation/AnimNotifies/AV_AnimNotifyState_SendGameplayEventBySweep.h"
...
TArray<FHitResult> UAV_AnimNotifyState_SendGameplayEventBySweep::PerformSweep(USkeletalMeshComponent* MeshComp) const
{
	TArray<FHitResult> Hits;
	
	// Check that Engine and World are valid
	if (!IsValid(GEngine)) return Hits;
	const UWorld* World = GEngine->GetWorldFromContextObject(MeshComp, EGetWorldErrorMode::LogAndReturnNull);
...
void UAV_AnimNotifyState_SendGameplayEventBySweep::SendEventsToActors(TArray<FHitResult> Hits, const USkeletalMeshComponent* MeshComp) const
{
	const UAbilitySystemComponent* AbilitySystem = UAbilitySystemBlueprintLibrary::GetAbilitySystemComponent(MeshComp->GetOwner());
	...
	for (FHitResult Hit : Hits)
	{
		...
		UAbilitySystemBlueprintLibrary::SendGameplayEventToActor(Hit.GetActor(), GameplayEventTag, Payload);
	}
}

//~ UAnimNotifyState
FString UAV_AnimNotifyState_SendGameplayEventBySweep::GetNotifyName_Implementation() const
{
	...
	FString Name = FString::Printf(TEXT("Send Gameplay Event: %s, by Trace: %s, with Collision: %s"), *GameplayEventTag.ToString(), *TraceString, *CollisionsJoined);
...
void UAV_AnimNotifyState_SendGameplayEventBySweep::NotifyTick(USkeletalMeshComponent* MeshComp, UAnimSequenceBase* Animation, float FrameDeltaTime, const FAnimNotifyEventReference& EventReference)
{
	// Check for validity of mesh and owner
	if (!IsValid(MeshComp) || !IsValid(MeshComp->GetOwner())) return;

	const TArray<FHitResult> Hits = PerformSweep(MeshComp);
	SendEventsToActors(Hits, MeshComp);
}

#if WITH_EDITOR

EDataValidationResult UAV_AnimNotifyState_SendGameplayEventBySweep::IsDataValid(FDataValidationContext& Context) const
{
	EDataValidationResult Result = Super::IsDataValid(Context);

	if (GameplayEventTag == FGameplayTag::EmptyTag)
	{
		Context.AddError(FText::FromString(TEXT("GameplayEventTag can't be Empty")));
