...
/*static*/ UAV_PrimaryGameLayout* UAV_PrimaryGameLayout::GetPrimaryGameLayout(ULocalPlayer* LocalPlayer)
{
	if (LocalPlayer)
	{
		...
		{
			if (UAV_GameUIManagerSubsystem* UIManager = GameInstance->GetSubsystem<UAV_GameUIManagerSubsystem>())
			{
				if (const UAV_GameUIPolicy* Policy = UIManager->GetCurrentUIPolicy())
				{
					if (UAV_PrimaryGameLayout* RootLayout = Policy->GetRootLayout(CommonLocalPlayer))
					{
						return RootLayout;
					}
				}
